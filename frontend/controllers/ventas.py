...
class VentasController:
    def __init__(self, page, data_manager):
        ...
        self.productos = obtener_productos_disponibles(data_manager)
    ...
    def filtrar_productos_con_stock(self):
        ...
        return [
            fl.dropdown.Option(key=p['id'], text=p['nombre'])
    ...
    def handle_producto_seleccionado(self, e, producto_list, ventas_list, monto_input):
        ...
        if producto_seleccionado:
            venta_existente = next(
                (v for v in self.productos_venta if v['nombre'] == producto_seleccionado['nombre']),
                None,
            )
            if venta_existente:
    ...
    def actualizar_lista_ventas(self, ventas_list, monto_input):
        ...
        for venta in self.productos_venta:
            ventas_list.controls.append(
                fl.Row(
                    [
    ...
    def actualizar_total(self, monto_input):
        total = obtener_total_venta(self.productos_venta)
    ...
    def handle_vender(self, monto_input, producto_list, ventas_list):
        ...
        if self.productos_venta:
            ...
            registrar_venta(self.data_manager, venta_data, self.productos_venta)
